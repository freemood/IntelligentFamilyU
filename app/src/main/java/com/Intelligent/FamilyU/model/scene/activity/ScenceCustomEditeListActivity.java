...
import com.Intelligent.FamilyU.model.scene.presenter.ScenceCustomDeletePresenter;
 ...
 * 场景
 */
public class ScenceCustomEditeListActivity extends BaseFragmentActivity implements IScenceSaveView, IScenceCustomDeleteView {
    ...
    private ScenceCustomDeletePresenter mScenceCustomDeletePresenter = new ScenceCustomDeletePresenter(this, this);
    ...
    protected int getContentViewId() {
        return R.layout.activity_scence_etid_list;
    }
...
//        }
    ...
    @Subscribe(threadMode = ThreadMode.MAIN, sticky = true)
    public void getEvent(DeviceScenceEtidItem deviceScenceEtidItem) {
        conditionsList = deviceScenceEtidItem.getSceneExecConditions();
        operationsList = deviceScenceEtidItem.getOperations();
        pk = deviceScenceEtidItem.getPk();
        int conList = conditionsList.size();
        HashMap<String, Object> map = new HashMap<String, Object>();

        for (int i = 0; i < conList; i++) {
            map = new HashMap<String, Object>();
            SceneExecCondition sceneExecCondition = conditionsList.get(i);
            map.put("title", sceneExecCondition.getParamIndex());
            map.put("status", "1");
            mlist.add(map);
        }
        int opeList = operationsList.size();
        for (int i = 0; i < opeList; i++) {
            map = new HashMap<String, Object>();
            SceneOperation sceneOperation = operationsList.get(i);
            map.put("title", sceneOperation.getParamIndex());
            map.put("status", "1");
            mlistDown.add(map);
        }
        initDownAdapter();
    ...
    @OnClick({R.id.page_back, R.id.page_right_rl, R.id.btn_delete, R.id.dingshi_rl, R.id.add_up, R.id.add_down})
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_delete:
                mScenceCustomDeletePresenter.custonDelete(mSerialNo, pk);
                break;
    ...
    @Override
    public void showResult(ScenceCustomDeleteBean bean) {
        if (null == bean) {
            return;
        }
        if (bean.isResult()) {
            showToast(getResources().getString(R.string.home_scence_delete));
            finish();
        }
    }
